...
This project allows you to run ComfyUI workflows as a serverless API endpoint on the RunPod platform. This endpoint is configured for the **v03_retouch_api** workflow, which performs advanced facial feature retouching using Flux inpainting models.
...
1.  🐳 Choose one of the [available Docker images](#available-docker-images) for your serverless endpoint (e.g., `runpod/worker-comfyui:<version>-flux1-dev`).
...
This API has been simplified to accept only the required image and optional parameters without needing to provide the full workflow JSON. The workflow is constructed automatically on the server side using the **v03_retouch_api** template for facial feature retouching.

The workflow performs:
- Face landmark detection (eyebrows, eyes, pupils, lips)
- Person mask generation with accessories detection
- Grounding DINO SAM segmentation
- Flux inpainting with advanced mask composition
- Face parsing and retouching
...
```json
{
  "input": {
    ...
    "prompt": "blemishes, wrinkles, acne",
    "seed": 42,
    "steps": 20,
    "cfg": 1.01,
    "denoise": 0.4
...
{
  "input": {
    "input_image": "data:image/jpeg;base64,/9j/4AAQSkZJRgABAQEAAAAAAAD...",
    "prompt": "skin imperfections"
  }
}
```
...
| `input.input_image`       | String  | Yes      | Portrait/face image to be retouched. Can be a URL (http/https) or base64 encoded string (with optional data URI prefix).                  |
| `input.prompt`            | String  | No       | Text prompt for Grounding DINO segmentation (e.g., "blemishes", "wrinkles", "acne", "skin imperfections"). Default: "" (empty)           |
| `input.seed`              | Integer | No       | Seed for reproducible generation. Default: 0                                                                                               |
| `input.steps`             | Integer | No       | Number of sampling steps for inpainting. Default: 20                                                                                       |
| `input.cfg`               | Float   | No       | CFG scale value for guidance. Default: 1.01                                                                                                |
| `input.denoise`           | Float   | No       | Denoise strength for inpainting (0.0-1.0). Higher values = more change. Default: 0.4                                                      |
