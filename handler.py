...
def create_workflow_template(input_image_name="input.jpg", prompt="", seed=0, steps=20, cfg=1.01, denoise=0.4):
    """
    Creates a complete workflow template for the v03_retouch_api Flux inpainting pipeline.
    
    This workflow performs facial feature retouching using Flux models with:
    - Face landmark detection
    - Person mask generation
    - Grounding DINO SAM segmentation
    - Flux inpainting with mask composition
    
    Args:
        input_image_name (str): Name of the input image file
        prompt (str): Text prompt for inpainting (for node 16) and segmentation (for node 99)
        seed (int): Seed for KSampler
        steps (int): Number of sampling steps
        cfg (float): CFG scale
        denoise (float): Denoise strength for inpainting
    ...
    return {
        ...
        "44": {
            ...
            "_meta": {"title": "SolidMask"}
        },
        "46": {
            "inputs": {
                "vae_name": "fulx_fill_vae.safetensors"
            },
        ...
        "52": {
            ...
            "_meta": {"title": "LayerMask: MaskGrow"}
        },
        "53": {
            "inputs": {
                "x": 0,
        ...
        "68": {
            "inputs": {
                ...
                "stitch": ["78", 0],
                "inpainted_image": ["77", 0]
            },
            "class_type": "InpaintStitch",
            "_meta": {"title": "(OLD 💀, use the new ✂️ Inpaint Stitch node)"}
        },
        "71": {
            "inputs": {
                "channel": "red",
        ...
        "78": {
            ...
            "_meta": {"title": "(OLD 💀, use the new ✂️ Inpaint Crop node)"}
        },
        "80": {
            "inputs": {
                "x": 0,
...
def validate_input(job_input):
    ...
    if input_image is None:
        return None, "Missing 'input_image' parameter"
    
    # Validate image format (must be base64 string or URL)
    if not isinstance(input_image, str):
        return None, "'input_image' must be a base64 encoded string or URL"
    
    # Optional parameters with defaults
    prompt = job_input.get("prompt", "")
    seed = job_input.get("seed", 0)
    steps = job_input.get("steps", 20)
    cfg = job_input.get("cfg", 1.01)
    denoise = job_input.get("denoise", 0.4)
...
def handler(job):
    ...
    input_image_input = validated_data["input_image"]
    prompt = validated_data["prompt"]
    seed = validated_data["seed"]
    steps = validated_data["steps"]
    cfg = validated_data["cfg"]
    denoise = validated_data["denoise"]
    
    # Convert URL to base64 if needed
    try:
        # Check if input_image is a URL or base64
        if input_image_input.startswith(('http://', 'https://')):
            input_image_base64 = download_image_from_url(input_image_input)
        else:
            input_image_base64 = input_image_input
    except Exception as e:
    ...
    input_images = [
        {"name": input_image_name, "image": input_image_base64}
    ]
